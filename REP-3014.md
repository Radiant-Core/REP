...
- **Protocol ID**: GLYPH_BURN = 6
- **Reference Implementation**: 
  - radiant-mcp-server: `radiant_burn_token` MCP tool and REST endpoint
  - Photonic Wallet: burn UI and transaction construction
- **Features**: NFT burn, FT full/partial burn, photon recovery, burn proof verification
- **Test Coverage**: 73/73 tests passing in radiant-mcp-server tx-builder
...
Token UTXO (tokenRef, 1000 photons)
  ...
  Burn Transaction
       ↓
├── Output 0: Burn proof (0 photons)
└── Output 1: User change (995 photons, minus fees)
```
...
NFT singletons naturally support burns—if the singleton is not pushed to an output, the token is effectively burned. The burn proof is optional metadata.
...
- Burn amount ≤ input token balance (for FT)
...
```
GET /burns                          — List all burns
GET /burns?token=:ref               — Burns for specific token
GET /burns?address=:addr            — Burns by address
GET /tokens/:ref/supply             — Circulating supply (total - burned)
GET /tokens/:ref/burns              — Burn history for token
```
...
│  🔥 Burn Successful                                     │
├─────────────────────────────────────────────────────────┤
│  Token:    Example Token #42                            │
│  Amount:   500 TOKENS                                   │
│  Reason:   Redemption for merch                         │
│  TXID:     abc123...                                    │
│  Photons:  495 recovered                                │
└─────────────────────────────────────────────────────────┘
```
...
- Recovery impossible—clear warnings required
