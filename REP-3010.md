...
- **Activation Height**: Block 410,000 (mainnet/testnet)
- **Radiant Core Version**: v2.1.0
- **New Opcodes**: OP_BLAKE3 (0xee), OP_K12 (0xef), OP_LSHIFT (0x98), OP_RSHIFT (0x99), OP_2MUL (0x8d), OP_2DIV (0x8e)
- **Implemented Algorithms**: SHA256d, Blake3, KangarooTwelve (K12)
- **Implemented DAA Modes**: Fixed, Epoch, ASERT (fully on-chain via new opcodes)
- **Reference Implementations**: 
  - Glyph Miner (WebGPU shaders for all algorithms)
  - Photonic Wallet (contract deployment UI)
  - Radiant Core (consensus + validation)
- **Test Coverage**: 125/125 tests passing in Glyph Miner, 26/26 functional tests in Radiant Core

Currently, all dMint contracts use SHA256d with a fixed difficulty target. This proposal introduces:
- **4 POW algorithms**: SHA256d (existing), Blake3, KangarooTwelve (K12), Argon2id-Light - Reserved for future
...
| `0x03` | Argon2id-Light | Memory-hard | GPUs with 64MB+ VRAM | Reserved for future |
