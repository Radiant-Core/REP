...
- **Reference Implementation**: RXinDexer (electrumx/lib/glyph.py)
- **Envelope Detection**: Dual-phase (OP_PUSHINPUTREF for FT, OP_PUSHINPUTREFSINGLETON for NFT)
- **Style Support**: Both Style A (OP_RETURN) and Style B (OP_3) implemented
- **Parser**: On-chain v1 and v2 format detection with CBOR metadata extraction
- **Test Coverage**: 5,112 tokens indexed (130 FT, 4,778 NFT, 204 dMint)
- **Test Suite**: Integrated with RXinDexer glyph_index tests
- **CBOR Library**: cbor2 (Python) and cbor-x (JavaScript) for canonical encoding
- **Validation**: All test vectors verified in functional tests
