...
- **Protocol ID**: GLYPH_CONTAINER = 7
- **Reference Implementation**: Photonic Wallet (`packages/lib/src/container.ts`)
- **Features**: Container creation, item add/remove, child token relationships, validation
- **Utilities**: `createContainer()`, `addItemToContainer()`, `removeItemFromContainer()`, `validateContainer()`, `isContainer()`, `isChildToken()`
- **Test Coverage**: Integrated with Photonic Wallet lib tests
...
|-------|------|----------|---------|-------------|
| `type` | string | Yes | - | Container type (see §2) |
...
4. Build container → items mapping
...
GET /containers/:ref                   — Get container details
GET /containers/:ref/items             — List items in container
GET /containers/:ref/items?page=2      — Paginated items
GET /containers/:ref/stats             — Collection statistics
GET /containers/:ref/traits            — Trait definitions
GET /containers/:ref/traits/:name      — Trait value distribution
GET /tokens/:ref/container             — Get container for item
```
...
│  🖼️ My Collection                                        │
│  5,000 / 10,000 minted                                  │
│  Floor: 50,000 photons | Volume: 1B photons             │
├─────────────────────────────────────────────────────────┤
...
### Why Child → Parent Reference Direction?
