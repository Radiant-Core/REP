...
- **Dependencies**: REP-3006 must be implemented first
- **Vectors Ready**: AES-256-GCM, ChaCha20-Poly1305, scrypt test vectors defined
- **Note**: XChaCha20-Poly1305 vectors pending library selection
...
#### Mode A — Encrypted payload, public metadata (RECOMMENDED)

- Metadata (type/name/etc.) remains readable
- `content.*.hash` refers to **ciphertext**
- Wallets display a "🔒 Encrypted" badge until key is provided

#### Mode B — Encrypted payload + encrypted metadata (OPTIONAL, future)
...
- `chacha20poly1305` (IETF, 12-byte nonce) — widely available fallback
...
- `content.*.hash` MUST refer to ciphertext bytes (see §9)
...
1. plaintext → compress
2. compressed bytes → encrypt (AEAD)
3. store ciphertext
...
- visibly mark encrypted Glyphs (e.g., "🔒 Encrypted")
...
- validate commit↔reveal linkage as usual (REP-3001/3002)
