...
- **Protocol ID**: GLYPH_AUTHORITY = 10
- **Reference Implementation**: Photonic Wallet (Authority Manager UI)
- **Features**: Authority minting, delegation, revocation, verified creator badges, multi-sig support
- **Contract Support**: V2 dMint contracts with authority validation
- **Test Coverage**: Integrated with 82/82 Photonic Wallet tests
...
|-------|------|----------|-------------|
| `type` | string | Yes | Authority type (see §2) |
...
3. Child permissions ⊆ parent permissions
4. Child constraints ≥ parent constraints (more restrictive)
...
```
┌─────────────────────────────────────────────────────────┐
│  🎨 Alice's Art Collection                              │
│  ✓ Verified Creator                                     │
│  Authority: abc123... (valid, not revoked)              │
└─────────────────────────────────────────────────────────┘
```
...
GET /authorities/:ref                    — Get authority details
GET /authorities/:ref/chain              — Get authority hierarchy
GET /authorities/:ref/delegations        — List sub-authorities
GET /authorities/:ref/usage              — Usage stats (mints, etc.)
GET /authorities/project/:name           — Authorities for project
GET /containers/:ref/authorities         — Accepted authorities
POST /authorities/validate               — Validate authority for action
```
...
│  🔑 AUTHORITY TOKEN                                     │
├─────────────────────────────────────────────────────────┤
│  Project XYZ Issuer Authority                           │
│                                                         │
│  Type:        Issuer                                    │
│  Permissions: mint, update, delegate                    │
│  Scope:       Project XYZ Collection                    │
│  Constraints: Max 10,000 mints (5,234 used)             │
│  Expires:     Never                                     │
│  Transferable: Yes                                      │
│  Status:      ✓ Valid                                   │
└─────────────────────────────────────────────────────────┘
